...
/*
 * VFS hook compatibility helpers
 *
 * Some 4.19 SUSFS kernel patches call these helpers directly from VFS
 * files such as fs/open.c, fs/namei.c, and fs/stat.c. Keep the syscall
 * family IDs and helper prototypes visible from linux/susfs.h so callers
 * do not depend on local extern declarations.
 */
struct filename;
struct kstat;
struct stat;
